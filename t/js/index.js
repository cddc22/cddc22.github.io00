...
var rand= 转换操作.到文本(数学操作.取随机数(1000,9999));
function 主窗口_创建完毕(){
...
function 按钮1_被单击(){
	...
	网络操作1.发送网络请求("hash.class.php","post","json","a="+多组合编辑框1.取标题(0),15000);
	}else{
...
if(项目索引 != 0){
	switch(列表框1.取项目标记(0)){
	  case "searchdata":
		   ...
		   网络操作1.发送网络请求("hash.class.php","post","json","b="+ 列表框1.取项目标记(项目索引),15000);
		   break;
	  case "hashlist":
	       对话框1.显示等待框("正在获取播放地址中(15s)..");
		   网络操作1.发送网络请求("hash.class.php","post","json","c="+ 列表框1.取项目标记(项目索引),15000);
		   break;
	  default:
	      对话框1.信息框("提示","后端数据不匹配");
	}
